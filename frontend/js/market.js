async function loadMarket() {
    ...
    try {
        ...
        container.innerHTML = gigs.map(g => `
            <div class="bg-white border border-zinc-200 p-4 sm:p-5 rounded-md hover:border-zinc-300 transition-colors">
                ...
                <div class="flex flex-col sm:flex-row gap-2 mt-4">
                    ...
                    <button onclick="placeBid(${g.project_id})" class="w-full sm:w-auto bg-zinc-900 hover:bg-zinc-800 text-white text-sm font-medium px-4 py-2 sm:py-1.5 rounded-md transition-colors">Place Bid</button>
                </div>
            </div>
        `).join('');

...
async function loadMyGigs() {
    ...
    try {
                    ...
                    actionArea = `
                        <div class="bg-blue-50 border border-blue-100 p-4 rounded-md">
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                                <select id="rating-${g.contract_id}" class="w-full sm:w-auto text-sm border border-blue-200 rounded-md px-3 py-2 outline-none focus:border-blue-500 bg-white">
                                ...
                                <button onclick="markCompleted(${g.project_id}, ${g.contract_id}, ${g.freelancer_id})" class="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-md transition-colors">Verify Cash Payment</button>
                            </div>
                        </div>
                    `;
                ...
                container.innerHTML += `
                    <div class="bg-white border ${g.status === 'open' ? 'border-zinc-300' : 'border-zinc-200 opacity-90'} p-4 sm:p-5 rounded-md">
                        ...
                        <div class="pt-4 border-t border-zinc-100">
                            ${g.status === 'open' ? '<h4 class="text-xs font-semibold text-zinc-900 mb-2">Bids</h4>' : ''}
                            <div class="space-y-1">${actionArea}</div>
                        </div>
                    </div>
                `;
            }
        ...
        // 2. Fetch Bids & Work (Freelancer View)
        const bidsRes = await fetch(`${API}/api/bids/my/${user.id}`);
        const myBids = await bidsRes.json();
        
        const workContainer = document.getElementById('my-work-container');
        if (myBids.length === 0) {
            workContainer.innerHTML = `<div class="text-sm text-zinc-500 text-center py-10 border border-dashed border-zinc-200 rounded-md">You haven't placed any bids yet.</div>`;
        } else {
            workContainer.innerHTML = myBids.map(b => {
                let actionArea = '';
                let statusBadge = '';

                if (b.bid_status === 'pending') {
                    statusBadge = `<span class="bg-zinc-100 text-zinc-600 text-[10px] uppercase font-bold px-2 py-1 rounded-sm shrink-0">Bid Pending</span>`;
                    actionArea = `<p class="text-sm text-zinc-600">You bid <strong class="font-bold">₹${b.proposal_amount}</strong>. Waiting for ${b.client_first} to respond.</p>`;
                } else if (b.bid_status === 'rejected') {
                    statusBadge = `<span class="bg-red-50 text-red-700 text-[10px] uppercase font-bold px-2 py-1 rounded-sm shrink-0">Rejected</span>`;
                    actionArea = `<p class="text-sm text-zinc-500">Your bid was not accepted.</p>`;
                } else if (b.bid_status === 'accepted') {
                    if (b.project_status === 'in_progress') {
                        statusBadge = `<span class="bg-blue-50 text-blue-700 text-[10px] uppercase font-bold px-2 py-1 rounded-sm shrink-0">In Progress</span>`;
                        actionArea = `<p class="text-sm text-blue-800">You are hired for <strong class="font-bold">₹${b.contract_amount}</strong>! Complete the work for ${b.client_first}.</p>`;
                    } else if (b.project_status === 'completed') {
                        statusBadge = `<span class="bg-emerald-50 text-emerald-700 text-[10px] uppercase font-bold px-2 py-1 rounded-sm shrink-0">Completed</span>`;
                        if (b.has_rated_client === 0) {
                            actionArea = `
                                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 mt-2">
                                    <select id="rate-client-${b.contract_id}" class="w-full sm:w-auto text-sm border border-emerald-200 rounded-md px-3 py-2 outline-none focus:border-emerald-500 bg-emerald-50 text-emerald-800">
                                        <option value="" disabled selected>Rate Client (${b.client_first})</option>
                                        <option value="5">⭐⭐⭐⭐⭐ (5) Paid instantly</option>
                                        <option value="4">⭐⭐⭐⭐ (4) Good</option>
                                        <option value="3">⭐⭐⭐ (3) Average</option>
                                        <option value="2">⭐⭐ (2) Slow payment</option>
                                        <option value="1">⭐ (1) Terrible / Scam</option>
                                    </select>
                                    <button onclick="rateClient(${b.contract_id}, ${b.client_id})" class="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-4 py-2 rounded-md transition-colors">Submit Rating</button>
                                </div>
                            `;
                        } else {
                            actionArea = `<p class="text-sm text-emerald-600 font-medium">Gig completed and client rated!</p>`;
                        }
                    }
                }

                return `
                    <div class="bg-white border border-zinc-200 p-4 sm:p-5 rounded-md">
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                            <div>
                                <h3 class="text-base font-semibold text-zinc-900">${b.title}</h3>
                                <p class="text-xs text-zinc-500 mt-0.5 cursor-pointer hover:text-accent transition-colors" onclick="showPublicProfile(${b.client_id})">Client: ${b.client_first} ${b.client_last} • Deadline: ${formatDate(b.deadline)}</p>
                            </div>
                            ${statusBadge}
                        </div>
                        <div class="pt-4 border-t border-zinc-100">
...
async function rateClient(contract_id, client_id) {
    const rating = document.getElementById(`rate-client-${contract_id}`).value;
    if(!rating) return alert("Please select a rating.");
    
    showLoader("Submitting rating");
    try {
        const res = await fetch(`${API}/api/gigs/rate`, {
            method: 'POST', headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ contract_id, reviewer_id: user.id, reviewee_id: client_id, rating: parseInt(rating) })
        });
        const data = await res.json();
        if(data.success) { alert("Rating submitted successfully!"); loadMyGigs(); }
        else alert(data.error);
